...
// FUNCTION UNWRAP RESULT SEARCH
    ...
    let callback = function(mutationsList, observer) {
      for (let mutation of mutationsList) {
        if (mutation.type === 'childList') {
          if (search.scrollHeight >= window.innerHeight && !search.classList.contains('scroll')) {
  ...
  if (document.querySelector('.community-preview-buttons')) {
    let changeParentEl = () => {
      ...
      if (window.innerWidth <= 768 && buttonsWrap.parentElement.classList.contains('community-preview')) {
